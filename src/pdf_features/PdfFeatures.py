...
from pdf_features.PdfToken import PdfToken, _tokens_by_top_cache
...
class PdfFeatures:
    def __init__(
        ...
        # ── Pre-sort tokens by top for O(log n) same-line / below lookups ──
        for page in self.pages:
            _tokens_by_top_cache[id(page.tokens)] = sorted(page.tokens, key=lambda t: t.bounding_box.top)
        self.get_modes()
        self.get_mode_font()
        self.get_tokens_context()
        # Clean up cache entries for this PdfFeatures instance (prevent memory leaks)
        for page in self.pages:
            _tokens_by_top_cache.pop(id(page.tokens), None)
    ...
    def get_modes(self):
        ...
        for page in self.pages:
            sorted_tokens = _tokens_by_top_cache.get(id(page.tokens), page.tokens)
            sorted_tops = [t.bounding_box.top for t in sorted_tokens]

            for token in page.tokens:
                bottom = token.bounding_box.bottom
                right = token.bounding_box.right

                # ── Find closest token below using binary search ──
                idx = bisect.bisect_right(sorted_tops, bottom)
                if idx < len(sorted_tokens):
                    closest_below = sorted_tokens[idx]
                    line_spaces.append(int(closest_below.bounding_box.top - bottom))

                # ── Same-line tokens to the right (now O(log n)) ──
                on_the_right = [
                    lt for lt in token.get_same_line_tokens(page.tokens)
                    if right < lt.bounding_box.left
                ]
                if not on_the_right:
