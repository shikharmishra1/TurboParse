...
_tokens_by_top_cache: dict[int, list] = {}
...
class PdfToken:
    ...
    def get_same_line_tokens(self, page_tokens: list["PdfToken"]):
        """Return tokens on the same line using binary search on pre-sorted tokens.

        Uses _tokens_by_top_cache (populated by PdfFeatures.__init__) for O(log n)
        lookup instead of O(n) linear scan.
        """
        sorted_tokens = _tokens_by_top_cache.get(id(page_tokens))
        if sorted_tokens is None:
            ...
            return [
                ...
                if top <= t.bounding_box.top < (top + height)
                or top < t.bounding_box.bottom <= (top + height)
            ]

        top = self.bounding_box.top
        height = self.bounding_box.height
        lo = top - height  # generous lower bound
        hi = top + height  # generous upper bound

        # Binary search for the range
        keys = [t.bounding_box.top for t in sorted_tokens]
        left = bisect.bisect_left(keys, lo)
        right = bisect.bisect_right(keys, hi)

        # Filter the narrow slice (usually just a few tokens)
        return [
            t for t in sorted_tokens[left:right]
            if top <= t.bounding_box.top < (top + height)
            or top < t.bounding_box.bottom <= (top + height)
        ]
