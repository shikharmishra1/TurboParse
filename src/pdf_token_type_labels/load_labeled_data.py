...
def load_labeled_data(pdf_labeled_data_root_path: str, filter_in: str = None, limit: int = None) -> list[PdfFeatures]:
    if filter_in:
    ...
    for dataset_name, dataset_path in loop_datasets(token_type_labeled_data_path, filter_in):
        ...
        for dataset, pdf_name in dataset_pdf_name:
            try:
                ...
                print(f"  Skipping {pdf_name}: {e}")

    if limit is not None:
        pdfs_features = pdfs_features[:limit]
